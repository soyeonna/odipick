...
# 동 이름과 구가 어긋나면 잡는다 (지도·지역 필터가 틀어진다)
# 같은 이름이 두 구에 있는 동네(용호동·장동 등)는 넣지 않는다
DONG_GU = {'봉명동':'유성구','구암동':'유성구','전민동':'유성구','노은동':'유성구','지족동':'유성구',
 '신성동':'유성구','도룡동':'유성구','관평동':'유성구','덕명동':'유성구','문지동':'유성구','갑동':'유성구',
 '원신흥동':'서구','도안동':'서구','관저동':'서구','둔산동':'서구','괴정동':'서구','탄방동':'서구',
 '월평동':'서구','갈마동':'서구','만년동':'서구','가장동':'서구','도마동':'서구','정림동':'서구',
 '대흥동':'중구','은행동':'중구','선화동':'중구','문화동':'중구','대사동':'중구','부사동':'중구',
 '유천동':'중구','태평동':'중구','오류동':'중구','석교동':'중구',
 '중리동':'대덕구','송촌동':'대덕구','법동':'대덕구','신탄진동':'대덕구','석봉동':'대덕구',   # 삼정동은 동구·대덕구 양쪽에 있어 판정에서 뺀다
 '대화동':'대덕구','오정동':'대덕구','비래동':'대덕구','읍내동':'대덕구',
 '용전동':'동구','판암동':'동구','대동':'동구','자양동':'동구','가양동':'동구',
 '신흥동':'동구','인동':'동구','정동':'동구','원동':'동구','삼성동':'동구'}

HOOK = re.compile(r'[‼❗️🔥💖🤍]|!!|!$|공유|저장하기|태그|팔로우|이벤트|미쳤|실화|주목|등장|떴|찾음|(?<!한국 )(?<!국내 )(?<!대전 )최초|레전드|역대급|난리|대란|무조건|찐맛집')
...
for p in P:
    ...
    _d = re.match(r'([가-힣]+동)', str(p.get('area') or ''))
    if _d and p.get('gu') and p['gu'] != '대전근교':
        _want = DONG_GU.get(_d.group(1))
        if _want and _want != p['gu']: add('동네와 구가 안 맞음', f"{p['n']} ({_d.group(1)} = {_want}인데 {p['gu']})")
...
HARD = {'동네와 구가 안 맞음', '릴스 문구 남음', '대전에서 먼 좌표', '대전여행인데 대전 밖', '엉뚱한 업종 연결', '이름 중복', '카카오번호 중복', '릴스번호 중복', '구글장소 중복', '화면 기능 빠짐', '공주픽인데 릴스 근거 없음'}
fail = False
